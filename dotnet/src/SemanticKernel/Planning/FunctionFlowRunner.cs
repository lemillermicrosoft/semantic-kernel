            ...
            foreach (XmlNode o in solution)
            {
                ...
                foreach (XmlNode o2 in o.ChildNodes)
                {
                    ...
                    if (o2.Name.StartsWith(FunctionTag, StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        if (!string.IsNullOrEmpty(functionName) && this._context.IsFunctionRegistered(skillName, functionName, out var skillFunction))
                        {
                            ...
                            if (o2.Attributes is not null)
                            {
                                ...
                                {
                                    this._context.Log.LogTrace("{0}: processing attribute {1}", parentNodeName, attr.ToString());
                                    if (attr.InnerText.StartsWith("$", StringComparison.InvariantCultureIgnoreCase))
                                    {
                                        ...
                                        if (attrValues.Length > 0)
                                        {
                                            ...
                                            {
                                                if (this._context.Variables.Get(attrValue[1..], out var variableReplacement))
                                                {
                            ...
                            foreach (var (key, _) in functionVariables)
                            {
                                ...
                                {
                                    this._context.Variables.Set(key, value);
                                }
                            }

                            _ = this._context.Variables.Update(result.ToString());
                            if (!string.IsNullOrEmpty(variableTargetName))
                            {
                                this._context.Variables.Set(variableTargetName, result.ToString());
                            }

                            if (!string.IsNullOrEmpty(appendToResultName))
                            {
                                _ = this._context.Variables.Get(Plan.ResultKey, out var resultsSoFar);
                                this._context.Variables.Set(Plan.ResultKey,
                        ...
                        {
                            this._context.Log.LogTrace("{0}: appending function node {1}", parentNodeName, skillFunctionName);
